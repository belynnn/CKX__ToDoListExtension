document.addEventListener('DOMContentLoaded', function () {
    ...
    const updateView = () => {
        const dTasksList = document.getElementById("d_tasks_list");

        var child = dTasksList.lastChild;
        while(child) {
            dTasksList.removeChild(child);
            child = dTasksList.lastChild;
        }
        ...
        taskArr.forEach((Element, index) => {
            ...
            taskDo.setAttribute("class", "btn-task-do btn");
            ...
            d_tasks_list.appendChild(newTask);
        });
    }

    const addTask = (isDone) => {
        const task = document.getElementById("ipt_task").value;
        if(task === null || task.trim() === "") return;
        taskArr.push({task, isDone});
        localStorage.setItem("savedTasks", JSON.stringify(taskArr));
        updateView();

        const taskInput = document.getElementById("ipt_task");
        taskInput.value = "";
    }
    ...
    document.getElementById("btn_submit_task").addEventListener("click", () => addTask(false));

    document.getElementById("btn_clear_task").addEventListener("click", () => {
        localStorage.clear();
        taskArr = [];
        updateView();
    })
